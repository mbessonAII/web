<?php include("connexionbdd.php");?>


<?php
  $select = $_POST['yearinput'];
  if(is_numeric($select) && $select>0)//verif si c'est un nombre et si il est strictement positif
  $reponse = $bddGalerie -> query('SELECT * FROM galerie01 WHERE YEAR(debut) ='.$select);
?>


<!-- Affichage images -->
<?php
  echo'<div class="container">
        <div class="row">
		      <div class="list-group gallery">';

  $unseulpassage = true;
  while ($donnees = $reponse->fetch())
  {
    if($unseulpassage == true)
    {
      $unseulpassage = false;
    }

    echo'
    <div class="col-xs-6 col-sm-4 col-md-3 col-lg-3">
        <a class="thumbnail fancybox" rel="ligthbox" href="images/galerie/'.$donnees['id_im'].'.jpeg">
            <img class="img-responsive" alt="" src="images/galerie/'.$donnees['id_im'].'_min.jpeg">
            <div class="text-center">
                <small class="text-muted">'.$donnees['legende'].'</small>
            </div> <!-- text-center / end -->
        </a>
    </div> <!-- col-6 / end -->
    ';
  }

  if(($donnees == false) && ($unseulpassage == true))//si on enleve $unseulpassage, on passe tout le temps ici
  {
    echo 'Pas d\'images';
  }
  $reponse->closeCursor();//termine le traitement de la requête*/

  echo'</div> <!-- list-group / end -->
      </div> <!-- row / end -->
    </div> <!-- container / end -->'
?>
